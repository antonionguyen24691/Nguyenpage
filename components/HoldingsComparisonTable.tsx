...
export default function HoldingsComparisonTable({ dates, rows }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-white/70 bg-white/80">
      <div
        className="grid gap-3 border-b border-outline-variant/40 px-5 py-4 text-xs font-bold uppercase tracking-[0.18em] text-on-surface-variant"
        style={{ gridTemplateColumns: `1.1fr repeat(${dates.length}, 0.75fr) 0.8fr` }}
      >
        <span>Mã</span>
      ...
      <div className="max-h-[360px] overflow-y-auto">
        {rows.slice(0, 20).map((row) => (
          <div
            key={row.stock_code}
            className="grid gap-3 border-b border-outline-variant/20 px-5 py-4 text-sm text-on-surface last:border-b-0"
            style={{ gridTemplateColumns: `1.1fr repeat(${dates.length}, 0.75fr) 0.8fr` }}
          >
